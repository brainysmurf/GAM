"""
This is code built upon old gam code that improves things
"""
...
def getGamPath():
    """
    Return the path to where we are located
    """
...
def doGetUserInfo(user_email=None):
    # we need credentials in self
    if user_email.startswith('uid:'):
        user_email = user_email[ len('uid:')+1: ]
...
def doGetDomainInfo():
    ...
    with manager.build_calls('directory') as key_builder:

        key_builder('users').\
            function_list().\
            update_kwarg(fields=u'users(customerId)', customer=customerID.customerId, sortOrder=u'DESCENDING').\
            set_default(customerID.customerId).\
            unpack_lambda(lambda result: result[u'users'][0][u'customerId'])

    with manager.build_calls('admin-settings') as key_builder:
